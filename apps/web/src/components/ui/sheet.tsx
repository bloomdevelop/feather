  ...
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[closed=]:duration-300 data-[expanded=]:duration-500 data-[expanded=]:animate-in data-[closed=]:animate-out",
  {
    variants: {
      position: {
        ...
        left: "inset-y-0 left-0 h-full w-full border-r data-[closed=]:slide-out-to-left data-[expanded]:slide-in-from-left sm:max-w-sm",
        right:
          "inset-y-0 right-0 h-full w-full  border-l data-[closed=]:slide-out-to-right data-[expanded=]:slide-in-from-right sm:max-w-sm",
      },
    },
    defaultVariants: {
      position: "right",
  ...
  return (
    <SheetPortal position={local.position}>
      ...
      <SheetPrimitive.Content
        ...
        {...others}
      >
        <SheetHeader class="sticky top-0 flex flex-row justify-between items-center w-full z-40 bg-background border-b-muted shadow-md p-2">
          <SheetTitle>{local.title}</SheetTitle>
          <SheetPrimitive.CloseButton class="rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
            <svg
